...
function App() {
  ...
  // Filter on countries
  const filteredCountries = !singleCountry.length ? countries
        .filter((country) => (
           country.name.common
            .toLowerCase()
    ...
    const countriesToDisplay = filteredCountries.map((filteredCountry, i) => {
        ...
        return <CountryName i={i} filteredCountry={filteredCountry} setSingleCountry={setSingleCountry} />
         } else {
  ...
  return (
    <div>
      ...
      <ul>
        {searchTerm.length === 0 ? null : filteredCountries.length > 10
          ? "Too many results"
          : countriesToDisplay}
      </ul>
    </div>
